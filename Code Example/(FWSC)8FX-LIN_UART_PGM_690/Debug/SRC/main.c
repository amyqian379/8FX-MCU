...
/*********************************************************************************************/	
	...
	{
		LIN_UART_Init(); 	// uart initialization
		SSR_RIE = 1;		// Enables receive interrupts
...
{
    PLLC = 0xA0;    // main cr pll x 2.5 = 4 x 2.5 = 10MHz
    SYCC = 0xFC;
    while (SYCC != 0xFC);
}

/*********************************************************************************************/
/*Module name:   LIN_UART_Init                                                               */
...
{
	AIDRL=0xFF;		// Port input enabled
	DDR4 = 0x00;	// SIN used as a LIN-UART serial data input pin
	SMR = 0x05;		// Mode 0: Asynchronous (Normal mode), enable SOE, LIN-UART reset
	SCR = 0x13;		// 8bit, 1stopbit, no parity, enable transmit and receive
	BGR0 = 86;		// Baud rate: 115200bps	
	SSR = 0x00;		// clear flags	
}
...
{	
	if(SSR_FRE || SSR_ORE)
	{
	//	SCR_CRE = 0;			// clear error flag
	    SCR |= 0x04;        // clear error flag
	}
	switch(Flag_Timeout)
	{
		case 1 : t1 ++;
	...
	if(SSR_RDRF)
	{
		...
		switch(counter)
		{
			case 1 : ReciveData.command = RDR_TDR;
					 Flag_Timeout = 1;		
					 break;
			case 2 : ReciveData.address = RDR_TDR;	
					 Flag_Timeout = 2;		 
					 break;
			case 3 : ReciveData.address <<= 8;
					 ReciveData.address |= RDR_TDR;
					 Flag_Timeout = 3;	
					 break;
			case 4 : ReciveData.datalen = RDR_TDR;
					 Flag_Timeout = 4;	
					 break;
			case 5 : ReciveData.datalen <<= 8;
					 ReciveData.datalen |= RDR_TDR;	
					 t1 = 0;
    				 t2 = 0;
    				 t3 = 0;
    				 t4 = 0;
	...
	switch(ReciveData.command)
	{
		case 0x01 : // erasing operation
					...
					if(ReciveData.address >= 0x8000)
					{
						...
						Specialarea.USER_NVR = ReciveData.datalen >> 8;
						...
						// NVR Value
						//Specialarea.USER_NVR &= 0x60;
						//Specialarea.NVR_CRTH |= Specialarea.USER_NVR;
		...
		case 0x04 : // reading operation
					...
					{		
						...
						WDTC = 0x35;// clear watchdog
						F_Timeout = 0;
						RDR_TDR = Flash_Read(ReciveData.address+readdatacnt,1);
						readdatacnt++;
...
void LIN_Transmit (void)
{
	if(SSR_TDRE)
	{	
		while(!SSR_TDRE);	// wait until transmit data register is empty
		RDR_TDR = ReciveData.Ack.result;		 // transmit the data of error information to PC side for flash security has been enabled	

...
en_result_t UsbDevice_ReceiveData(stc_usbn_t* pstcUsb, stc_usbn_endpoint_data_t* pstcEndpoint, uint8_t* pu8Buffer, uint32_t u32DataSize, en_usbsend_mode_t enMode)
{
    ...
    /* 20170427 KOZU check DRQ flag */
    if(USB_EPNS_DRQ_ISSET(pstcEndpoint))
    {
        /* If the DRQ bit is set, clear DRQ bit */
        USB_EPNS_DRQ_CLEAR(pstcEndpoint);
    }
...
static void SetupRxCallback(stc_usbn_t* pstcUsb)
{
    ...
    stc_usbdevice_reportdescriptor_t *pstHidDes;
    stc_usbdevice_reportdescriptor_t *pstReportDes;
...
//        {
//            //printf("SET_IDLE: 0x%2x 0x%02x 0x%04x 0x%04x 0x%04x\r\n", stcSetup.bmRequestType, stcSetup.bRequest, stcSetup.wValue, stcSetup.wIndex, stcSetup.wLength);
//            //recive SET_IDLE
...
//    }
    ...
    case 0: // Device
      switch(stcSetup.bRequest) 
      {
        case GET_DESCRIPTOR:
            switch((stcSetup.wValue) >> 8)
            {
                ...
                case 3:    // String Descriptor
                  ...
                  switch ((stcSetup.wValue) & 0xFF)
                  {
                      case 0:
                          ...
                          u8DataToSend[2] = 0x09;
                          u8DataToSend[3] = 0x04;
                          if(u16Length > 4) {u16Length = 4;}
                          UsbDevice_SendData(pstcUsb, pstcUsbDeviceIntern->pstcEp0IN, u8DataToSend, u16Length, UsbPOLL);
                          break;
#if (WIN10_MOBILE_ENABLE)
                      case MS_OS_DSCR_STRING_INDEX:
                          for(u8Size=0; u8Size<pstcUsbDeviceIntern->pu8MicrosoftOSDescriptors[0]; u8Size++)
                          {
                                u8DataToSend[u8Size] = pstcUsbDeviceIntern->pu8MicrosoftOSDescriptors[u8Size];
                          }
                          if(u16Length > u8DataToSend[0]) {u16Length = u8DataToSend[0];}
                          UsbDevice_SendData(pstcUsb, pstcUsbDeviceIntern->pstcEp0IN, u8DataToSend, u16Length, UsbPOLL);
                          break;
#endif /* WIN10_MOBILE_ENABLE */
                      default:
                          if (((stcSetup.wValue) & 0xFF) <= pstcUsbDeviceIntern->u8StringDescriptorCount)
                          {
                              if (pstcUsbDeviceIntern->pstcStringDescriptors[((stcSetup.wValue)&0xFF)-1].pu8String != NULL)
                              {
                                  u8DataToSend[0] = 2;
                                  u8DataToSend[1] = 0x03;
                ...
                case 0x21:
                  u16Length = stcSetup.wLength;
                  interface_number = (stcSetup.wIndex & 0x03);
                  //printf("21 %d\r\n", interface_number);
                  pstHidDes = &(pstcUsbDeviceIntern->astcHIDDescriptors[interface_number]);
                  if(pstHidDes->pu8Descriptor != NULL)
                  {
                    if(u16Length > pstHidDes->u16Size) u16Length = pstHidDes->u16Size;
                    memcpy(u8DataToSend, pstHidDes->pu8Descriptor, u16Length);
                    UsbDevice_SendData(pstcUsb, pstcUsbDeviceIntern->pstcEp0IN, u8DataToSend, u16Length, UsbPOLL);
                  }
                  break;
                ...
                case 0x22: // Report Descriptor
                  u16Length = stcSetup.wLength;
                  interface_number = (stcSetup.wIndex & 0x0003);
                  //printf("22 %d\r\n", interface_number);
                  pstReportDes = &(pstcUsbDeviceIntern->astcReportDescriptors[interface_number]);
                  if(u16Length > pstReportDes->u16Size) u16Length = pstReportDes->u16Size;
                  pstcUsbDeviceIntern->pstcEp0IN->pfnRxTxBlockCallback = ControlTxCallback;
                  UsbDevice_SendData(pstcUsb, pstcUsbDeviceIntern->pstcEp0IN, pstReportDes->pu8Descriptor, u16Length, UsbPOLL);
                  break;
                
                default:
                  //printf("USB Request Unknown Descriptor\r\n");
                  //printf("GET_DES:0x%2x 0x%02x 0x%04x 0x%04x 0x%04x\r\n", stcSetup.bmRequestType, stcSetup.bRequest, stcSetup.wValue, stcSetup.wIndex, stcSetup.wLength);
      ...
      default:
        ...
        //printf("USB Request Unknown\r\n");
      }
      break;
    
    case 1: // Class
      //printf("Class: 0x%2x 0x%02x 0x%04x 0x%04x 0x%04x\r\n", stcSetup.bmRequestType, stcSetup.bRequest, stcSetup.wValue, stcSetup.wIndex, stcSetup.wLength);
    ...
    case 2: // Vendor
      ...
      {
        /* MS OS */
#if (WIN10_MOBILE_ENABLE != 0)
        case CY_US_MS_OS_VENDOR_CMD:
          if(CY_US_OS_COMPATIBILITY_ID == stcSetup.wIndex)
          {
            u16Length = (((uint16_t)(pstcUsbDeviceIntern->pu8MSOScompatibilityID[1]) << 8) | ((uint16_t)(pstcUsbDeviceIntern->pu8MSOScompatibilityID[0])));
            if(stcSetup.wLength < u16Length)
            {
              u16Length = stcSetup.wLength;
            }
            UsbDevice_SendData(pstcUsb, pstcUsbDeviceIntern->pstcEp0IN, pstcUsbDeviceIntern->pu8MSOScompatibilityID, u16Length, UsbPOLL);
          }
          else if(CY_US_OS_EXTENDED_PROP == stcSetup.wIndex)
          {
            u16Length = (((uint16_t)(pstcUsbDeviceIntern->pu8ExtendedPrpertyDescriptor[1]) << 8) | ((uint16_t)(pstcUsbDeviceIntern->pu8ExtendedPrpertyDescriptor[0])));
            if(stcSetup.wLength < u16Length)
            {
                u16Length = stcSetup.wLength;
            }
            UsbDevice_SendData(pstcUsb, pstcUsbDeviceIntern->pstcEp0IN, pstcUsbDeviceIntern->pu8ExtendedPrpertyDescriptor, u16Length, UsbPOLL);
          }
          break;
#endif /* (WIN10_MOBILE_ENABLE != 0) */
        /* Vendor request */
        default:
          //printf("Vendor: 0x%2x 0x%02x 0x%04x 0x%04x 0x%04x\r\n", stcSetup.bmRequestType, stcSetup.bRequest, stcSetup.wValue, stcSetup.wIndex, stcSetup.wLength);
          for(u8Size=0; u8Size<USBDEVICE_MAXCLASSES; u8Size++)
          {
            if(pstcUsbDeviceIntern->stcUsbClasses[u8Size].pfnVendorRequestCallback != NULL)
            {
              pstcUsbDeviceIntern->stcUsbClasses[u8Size].pfnVendorRequestCallback(pstcUsb, &stcSetup);
            }
          }
          break;
      }
...
en_result_t UsbDevice_RegisterVendorClass(stc_usbn_t* pstcUsb, stc_usbdevice_class_config_t* pstcUsbClassConfig, uint8_t** ppu8Buffer, uint8_t* pu8InterfaceLength)
{
    ...
    stc_usbdevice_intern_data_t* pstcUsbDeviceIntern;
    pstcUsbDeviceIntern = UsbDeviceGetInternDataPtr(pstcUsb);
    /* Get all configuration descriptor length */
    u16TotalLen = pstcUsbDeviceIntern->pu8ConfigDescriptor[2] + pstcUsbDeviceIntern->pu8ConfigDescriptor[3]*256;
    
    for(u16i = 0;u16i < u16TotalLen;)
    {
        if (pstcUsbDeviceIntern->pu8ConfigDescriptor[u16i+1] == USBDESCR_INTERFACE)
        {
                ...
                ((pstcUsbClassConfig->u8InterfaceNumber == 0xFF) || (pstcUsbClassConfig->u8InterfaceNumber == pstcUsbDeviceIntern->pu8ConfigDescriptor[u16i+2])))
            {
                *ppu8Buffer = (pstcUsbDeviceIntern->pu8ConfigDescriptor + u16i);
                ...
                pstcUsbDeviceIntern->stcUsbClasses[pstcUsbDeviceIntern->pu8ConfigDescriptor[u16i+2]].pfnVendorRequestCallback = pstcUsbClassConfig->pfnVendorRequestCallback;
                // 20170414
                *pu8InterfaceLength = pstcUsbDeviceIntern->pu8ConfigDescriptor[u16i];
                u16i += pstcUsbDeviceIntern->pu8ConfigDescriptor[u16i];
                
                while(u16i < u16TotalLen)
                {
                    if (pstcUsbDeviceIntern->pu8ConfigDescriptor[u16i+1] == USBDESCR_INTERFACE) 
                    {
                        break;
...
void UsbDevice_Init(stc_usbn_t* pstcUsb, stc_usb_config_t* pstcUsbConfig, stc_usbdevice_config_t* pstcConfig)
{
    ...
    pstcUsbDeviceIntern->astcHIDDescriptors = pstcConfig->astcHIDDescriptors;
...
#if (WIN10_MOBILE_ENABLE)
    pstcUsbDeviceIntern->pu8MicrosoftOSDescriptors = pstcConfig->pu8MicrosoftOSDescriptors;
    pstcUsbDeviceIntern->pu8MSOScompatibilityID = pstcConfig->pu8MSOScompatibilityID;
    pstcUsbDeviceIntern->pu8ExtendedPrpertyDescriptor = pstcConfig->pu8ExtendedPrpertyDescriptor;
#endif /* WIN10_MOBILE_ENABLE */
